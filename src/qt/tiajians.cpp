...
{
    return QCoreApplication::translate("tiajians-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in tiajians.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in tiajians.qrc)
...
/** Class encapsulating Tiajians Core startup and shutdown.
...
/** Main Tiajians application object */
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // tiajians: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Tiajians Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(tiajians);
    Q_INIT_RESOURCE(tiajians_locale);
    ...
    /// 6. Determine availability of data directory and parse tiajians.conf
    ...
    // tiajians: links repeatedly have their payment requests routed to this process:
