...
# tiajians_hash
RUN git clone https://github.com/tiajians/tiajians_hash
RUN cd tiajians_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} tiajians
RUN useradd -u ${USER_ID} -g tiajians -s /bin/bash -m -d /tiajians tiajians
...
RUN mkdir /tiajians-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /tiajians-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /tiajians-src
