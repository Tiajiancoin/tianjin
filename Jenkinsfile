...
// which are part of the Tiajians repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="tiajians-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /tiajians-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "tiajians-tiajians/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from tiajians-tiajians/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'tiajians-tiajians/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from tiajians-tiajians/develop"
            }
          }

          if (hasCache) {
            sh "cd /tiajians-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /tiajians-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /tiajians-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /tiajians-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /tiajians-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /tiajians-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /tiajians-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
