...
*Setup instructions for a Gitian build of Tiajians Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Tiajians
...
- [Building Tiajians Core](#building-tiajians-core)
...
Clone the git repositories for Tiajians Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/tiajians/tiajians
...
in the Tiajians Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Tiajians Core
----------------

To build Tiajians Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Tiajians Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/tiajians/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/tiajians/tiajians
...
For example:
```bash
URL=https://github.com/crowning-/tiajians.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit tiajians=${COMMIT} --url tiajians=${URL} ../tiajians/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit tiajians=${COMMIT} --url tiajians=${URL} ../tiajians/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit tiajians=${COMMIT} --url tiajians=${URL} ../tiajians/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the tiajians git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../tiajians/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/tiajians
SIGPATH=/some/root/path/tiajians-detached-sigs

./bin/gbuild --url tiajians=${BTCPATH},signature=${SIGPATH} ../tiajians/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tiajians-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tiajians-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/tiajians-osx-build.assert
```
...
[tiajians/gitian.sigs](https://github.com/tiajians/gitian.sigs/) repository, or if that's not possible to create a pull
request.
